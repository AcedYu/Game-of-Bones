...
async function remainingCards() {
  ...
  for (var i = 0; i < res.cards.length; i++) {
    ...
    if (deckCardNameArray.includes(res.cards[i].name)) {
      ...
      let cardTemplate = $(`
      <div class="card card-collection card-cover h-100 overflow-hidden text-white bg-dark rounded-5 shadow-lg" style="background-image: url(${res.cards[i].image_url});">
      ...
      <div class="card-footer position-sticky">
        ...
        <button id="addToDeck" type="button" class="btn btn-success" cardId="${res.cards[i].id}">Add to Deck</button>
...
const addToDeck = async (event) => {
  ...
  var card_id = target.attr('cardId')
